...
export default function TripsPage() {
  ...
  // Trip routes yükle (RouteType.Trip = 1) - Public access
  useEffect(() => {
    loadTrips();
  }, []);
  ...
  const loadTrips = async () => {
    try {
      ...
      // Try to load trips (public access)
      try {
        const plans = await travelPlansApi.getByType(1); // RouteType.Trip = 1
        setTravelPlans(plans);
        setFilteredPlans(plans);
      } catch (apiError: any) {
        // If API requires auth, try getAll instead (might be public)
        if (apiError?.status === 401) {
          console.log('TripsPage: API requires auth, trying public endpoint...');
          try {
            const plans = await travelPlansApi.getAll();
            // Filter for Trip type (type = 1)
            const tripPlans = plans.filter(plan => plan.type === 1);
            setTravelPlans(tripPlans);
            setFilteredPlans(tripPlans);
          } catch (publicError) {
            console.error('Error loading trips:', publicError);
            setTravelPlans([]);
            setFilteredPlans([]);
          }
        } else {
  ...
  const handleCreateNew = () => {
    router.push('/routes/create?type=1'); // RouteType.Trip = 1
  };
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                  ...
                  <div className="relative flex-1">
                    <input
                      ...
                      placeholder="Search trip routes..."
                  ...
                  {/* Create New Trip Button - Most prominent */}
                  <button
            ...
            {isLoadingPlans ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-gray-500 mb-6">
                  {searchQuery ? 'No search results found.' : 'Create your first trip!'}
                </p>
                <button
                  onClick={handleCreateNew}
